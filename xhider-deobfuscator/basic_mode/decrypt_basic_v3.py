...
def decrypt_lua_basic_v3(input_file):
    try:
    ...
    def lua_unescape(s):
        # A simple unescaper for common Lua escapes
        # Lua matches: \a, \b, \f, \n, \r, \t, \v, \\, \", \', \[ \], \z
        # Also \ddd (decimal)
        
        # We can simulate this by walking the string
        res = []
        i = 0
        n = len(s)
        while i < n:
            ...
            if c == '\\':
                ...
                if next_c.isdigit():
                    ...
                    res.append(next_c) # Unrecognized escape, keep literal
                    i += 2
            else:
                res.append(c)
                i += 1
    ...
    while curr < len(X):
        ...
        if s is not None:
             ...
             print(f"Offset {curr} ({hex(curr)}): \"{safe_s}\"")
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        decrypt_lua_basic_v3(sys.argv[1])
    else:
        decrypt_lua_basic_v3("d:\\dec bot2\\basic_mode\\1765094582044242.lua")
