...
def decrypt_lua_basic_8419_v3(input_file):
    try:
    ...
    # Extract bytecode at 0x5f
    print("Extracting payload at 0x5f...")
    bytecode, b_len = decrypt_string(0x5f, X)
    
    if bytecode:
        print(f"Extracted payload length {len(bytecode)}")
        bc_path = input_file + ".luac"
        with open(bc_path, "wb") as f:
            f.write(bytecode)
        print(f"Saved payload to {bc_path}")
        
        # Dump strings from payload
        print("Strings in payload:")
        # Simple ASCII dumper
        min_len = 4
        current_str = ""
        for b in bytecode:
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        decrypt_lua_basic_8419_v3(sys.argv[1])
    else:
        decrypt_lua_basic_8419_v3("d:\\dec bot2\\basic_mode\\8419960403249811.lua")
