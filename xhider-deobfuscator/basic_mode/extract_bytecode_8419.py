...
def decrypt_lua_basic_8419_v2(input_file):
    try:
    ...
    # Extract bytecode at 0x5f (based on previous run output)
    print("Attempting to extract bytecode at 0x5f...")
    bytecode, b_len = decrypt_string(0x5f, X)
    
    if bytecode and bytecode.startswith(b'\x1bLua'):
        print(f"Extracted Bytecode, length {len(bytecode)}")
        bc_path = input_file + ".luac"
        with open(bc_path, "wb") as f:
            f.write(bytecode)
        print(f"Saved bytecode to {bc_path}")
    else:
        print("Bytecode extractions failed or signature mismatch.")
        if bytecode:
             print(f"First bytes: {bytecode[:10]}")

if __name__ == "__main__":
    if len(sys.argv) > 1:
        decrypt_lua_basic_8419_v2(sys.argv[1])
    else:
        decrypt_lua_basic_8419_v2("d:\\dec bot2\\basic_mode\\8419960403249811.lua")
