...
def decrypt_lua_basic_v5(input_file):
    try:
    ...
    # Robust regex for quoted string with escapes
    # local U="((?:[^"\\]|\\.)*)"
    u_match = re.search(r'local U="((?:[^"\\]|\\.)*)"', content)
    if not u_match:
        # Try simplified spacing with same regex
    ...
    print(f"DEBUG: Found U length {len(U_raw)} (raw)")
    ...
    # Targeted decryption based on calls in file
    decrypted_map = {}

    offsets = set()
    # Find all S(0x...) in original content
    ...
    # Note: Regex modified to handle spaces more flexibly
    new_content = re.sub(r'e\[S\(\s*(0x[0-9A-Fa-f]+)\s*\)\]', replacer, content)
...
if __name__ == "__main__":
    if len(sys.argv) > 1:
        decrypt_lua_basic_v5(sys.argv[1])
    else:
        decrypt_lua_basic_v5("d:\\dec bot2\\basic_mode\\1765094582044242.lua")
