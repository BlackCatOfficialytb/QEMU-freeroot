...
def parse_constants_table(content):
    """Locate `local _0x... = { ... }` and return the parsed list of items + the var name."""
    m = re.search(r'local\s+(_0x\w+)\s*=\s*\{', content)
    if not m:
        return None, None
    var = m.group(1)
...
def parse_b64_map(content):
    """Find the second `local _0x... = { k = expr, ... }` table."""
    matches = list(re.finditer(r'local\s+(_0x\w+)\s*=\s*\{', content))
...
def custom_b64_decode(byte_vals, b64_map):
    """Apply the hard_mode decode: shift -49 then base64-decode using map."""
    shifted = [(b - 49) % 256 for b in byte_vals]
    out_bytes = []
    buf = 0
    cnt = 0
    i = 0
...
def deobfuscate(input_file):
    ...
    for it in items:
        if it['type'] == 'string':
            ...
            decoded = custom_b64_decode(byte_vals, b64_map)
            try:
